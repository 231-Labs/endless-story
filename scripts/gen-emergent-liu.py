...
# ⚠ 種子對稱鐵則：這兩條 view 的文字會被引擎的 bond 分類器（tick.ts §2.95）掃描——
# 命中 INTIMATE 關鍵詞（一夜/枕/情人/舊情/雲雨…）會把該對「自動設成相許＋高溫度」。
# 為讓湧現訊號乾淨，兩條必須種在「同一檔」（此處皆 YEARNING：含「放不下」），
# 皆不相許、溫度相當，讓 run 自己決定她偏誰。改字時務必兩邊一起檢查，別讓一邊誤觸
# INTIMATE（那會把「她偏誰」偷偷寫死）。初夜/越了線 等事件不觸發，可保留。
VIEW_TO_JIN = (
