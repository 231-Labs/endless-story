...
### Showrunner 開新標的 / 退場舊標的（D5）

showrunner 心跳除了更新弧線，還會讀「現有標的＋卡司」決定要不要 `instantiate`（開一條新爭奪軸，
如「誰頂替出缺的乾生」）或 `retire`（收掉已塵埃落定的軸）。過守恆護欄（kind 須 ascii slug、不可撞
內建、seekers≥2 且在卡司內、導演標的上限 3、開標的每 3 tick 冷卻一次）。新標的下一 tick 自動被渴望、
被 spine 當內建標的一樣結算——世界因此能**長出原本沒有的衝突軸**，而不只在固定三條線輪。
log 看 `【世界推進】Showrunner 動了標的`。
