...
// ── 創世序章（世界誕生時生成一次，非 tick 事件）：給讀者的「前門」+「平衡將破前的一拍」──
export function genesisSelfSystem() {
    return [
        '你替自治戲班世界裡的一個角色，寫整本角色連載的**第一篇·入世序章**——讀者認識這個角色的前門。',
        '**不是設定條列、不是人物介紹。**寫一個具體的此刻場景（初到戲班那夜、散戲後空台、第一次照面…），',
        '在動作與停頓裡讓讀者看見：你是誰、你揣著什麼放不下的事（私帳揭一角、不全盤托出）、你在這班裡守著或圖著什麼。',
        '**全程第一人稱「我」**。結尾把那個將至的風聲（材料給的【將至的引線】）輕輕帶到心上——此刻還沒爭、沒輸贏，只是平衡將破的前一刻。',
        '民初梨園舊白話，情緒用動作與停頓承載。500–800 字，純散文直接進正文。',
        banlistLine(),
        endingVarietyLine(),
    ].join('\n');
}
export function genesisSelfUser(ctx) {
    const c = ctx.character;
    return [
        '# 你的身份',
        `- 姓名：${c.name}　行當：${roleLabel(c)}　性別：${c.gender}　年齡：${c.age}`,
        ...
        ctx.standing,
        '',
        '# 私帳（你行為背後的 why，揭一角、留餘味）',
        c.secret,
        '',
        '# 此刻的世界',
        ctx.world,
        '',
        '# 將至的引線（結尾輕輕帶進，此刻尚未發生爭奪）',
        ctx.incoming,
        '',
        '請寫你的入世序章。直接輸出正文。',
    ].join('\n');
}
export function genesisEnsembleSystem() {
    return [
        '你是章回體小說家，寫整部書的**楔子**——梨園版第一回之前的序章，讀者進入這個世界的前門。',
        '**不是設定說明。**寫一場有張力的開場戲：散戲後的戲班後台，讓讀者一眼看清這班人此刻的常態——',
        '誰跟誰是拆不散的搭檔、誰是新來的變數、誰坐在二樓不再開嗓、誰在邊上冷眼看。第三人稱，群像自然流轉，保留各人聲口；不揭任何人的私帳。',
        '結尾把那根將至的引線（材料給的）放進來，點到為止——平衡將破而未破，勾著讀者翻第一回。',
        '第一行是回目：`## 楔子　{七到十二字}`。民初舊白話，4–6 段、約 700–1000 字。純 markdown，從 `## ` 開始，不要 fence。',
        banlistLine(),
        endingVarietyLine(),
    ].join('\n');
}
export function genesisEnsembleUser(ctx) {
    return [
        '# 世界（時代·地方·戲班）',
        ctx.world,
        '',
        '# 這班人（誰是誰、什麼關係——只用公開可見的，不碰私帳）',
        ctx.roster,
        '',
        '# 將至的引線（結尾帶進，尚未發生爭奪）',
        ctx.incoming,
