...
> ⚠️ 若日後想讓「單軸也呈鐘型」(極端值更稀有、90 更珍貴),需改骰法為「取 k 次均勻的平均」。那會讓高門檻變成**指數級**稀有,§1.3 的 $E$ 與必應都要重算。**本公式假設每軸 uniform。**
...
**必應**:後端只重骰便宜的骰子(不叫 LLM、不上鏈、不生圖)直到達標,**命中才生成 + mint**,一次付清。它賣的是「**省下的平均硬抽花費 + 保證 + 即時**」。
...
- `margin = 0.85`(專案預設):必應比期望硬抽**略便宜** → 想要角色的人直接選它,消掉變異帶來的暴怒、營收平滑;單抽留給想賭便宜/求刺激的人。
  - `1.0` = 公道價(期望花費相同,但即時+保證)。
  - `1.3` = 溢價(把必應當高級便利服務)。
- **clamp ≥ basePrice**:必應永不低於單抽。
- 門檻越苛 → $E$ 越大 → 必應越貴。**所以這是 per-徵召 的值,不能用一個全站固定數字。**

實作:`packages/web/src/lib/recruit-pricing.ts`(`expectedDrawsToMeet` / `suggestedBulkPrice`)。
Admin 後台「招募」每列有「建議」鈕(依當前門檻即時算),頂部有「批次定必應」(一鍵套用到全部)。
...
對照(全 13 個現役徵召),平均達標抽數落在 **4.4(票房,機≥78)→ 27.2(花旦)**;必應(×0.85)約 **370 → 5090**。難度差 ~6 倍,印證「不可用固定值」。

### 1.5 注意

- $E$ 只取決於 `minAttributes`;**性別不入骰**(性別由 preview prompt 強制 + 鏈上 `check_voucher_requirements` 把關),不影響抽數。
- 門檻極苛(例如四軸全 ≥ 90)時 $E$ 會很大、必應會很貴 —— 此時考慮**調低門檻**或**抬高 basePrice**,而不是讓必應變天文數字。
