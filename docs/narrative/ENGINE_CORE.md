...
| 模組 | 管什麼 |
|---|---|
| `event-deck.ts` | 事件卡／世情動作 schema、可打牌集與可做之事（純函式）、確定性結算、導演自撰的驗收、決策 log |
...
**兩道穿過牌組天花板的門**（都靠同一組積木，所以都不會把寫入權還給模型）：

- **導演自撰一張**（`validateProposal`）：牌面上沒有合用的，導演可以用**同一套有限
  effect** 拼一張牌，但量級與限額由引擎執行（一卷 3 張、一日 1 張，`PROPOSAL_LIMITS`），
  且發錢／分紅／注資／結帳／進出班／見報／人心整類關死——能造錢的導演能把整季賴以成立
  的稀缺一次溶掉。越界連同**每一條理由**駁回並記進 `proposalsRefused`；通過的以
  `chosenBy: 'director-proposed'` 落 log，並**逐字帶上 `proposedCard`**（那張牌不在
  任何牌組檔裡，log 不帶它就重放不出來）。
- **角色自己造事件**（`acts` / `availableActsFor` / `playAct`）：報官、當眾翻臉、撂挑
  子、逐出班子這一類「一做就翻過去」的事，原本不屬於任何人——導演做不了（那是一個人的
  決定），班裡也沒有管道。`acts` 讓它成為**一張只有角色能打的牌**：引擎算出誰此時此地
  做得出什麼，beat prompt 只亮這幾張，回來的 id 再驗一次才落地，`chosenBy: 'character'`
  ＋ `actorId` 進同一本 log。後果一律走既有機制——`standing` effect 寫的是 `edges` ＋
  `chillBond`，不是新的口碑帳（同 §4 那條更正）。
