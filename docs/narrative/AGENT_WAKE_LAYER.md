...
- **時辰是鐘，拍是搬演——兩個節律，不可綁死**（實測後的關鍵修正）：
  先前「一日六拍」把兩者綁成一件，於是鏡像卷可以整整四個鐘頭一動也不動，
  看上去就是死的。人在四個鐘頭裡當然不只做一件事。故：時辰照現實走（永不
  快轉），**戲每隔 `beatIntervalMs`（預設三分鐘）搬演一次**，時辰邊界另必打一拍。
- **夜裡只在時辰邊界打拍**：引擎對夜拍一律「快轉、睡去整併」（零場景零拍），
  而入夜＋深宵在鏡像時間裡是**真實八小時**——每三分鐘叫一次只是拿真金白銀
  買一串空拍。夜裡改走邊界節律，天亮自然回到分鐘節律。
  空拍在拍流上什麼都不留、讀者分不清世界睡了還是程式壞了，故**每個夜時辰
  留一行世界話**（「夜深了，燈一盞盞暗下去」）。
- **失敗不凍世界，也不燒錢**：driver 只防堆疊（手上有未走完的拍就不排），
  不以「拍號沒變」封死——那會讓一記失敗的拍把世界永久凍住。連
  `AUTO_REST_AFTER_FAILURES`（3）拍走不動則自行歇班，錯在哪寫在 lastError。
- **控制列隨曆法分歧**：狀態與開關併為一枚章（走拍時報佇列、靜下來報
  活著／歇班，點一下換上下工）。鏡像卷**不給「連走 N 拍」**——一拍綁在
  時辰上，連下六拍只是叫同一個黃昏搬演六次，世界並不會走到明天；活世界
  要往前走靠「活著」與時辰邊界，手撥就是一拍（`叫一拍`）。排演卷維持
  原本的連走 N（那裡 tick 就是時間，連走名副其實）。
