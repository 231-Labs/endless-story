...
- 跑 `packages/cli/scripts/world-loop.ts`（`--interval` / `--max`,序列等每 tick 完成、永不重疊）。
...
- 所以這個 service **只需要 `WORLD_LOOP_URL` + `TICK_LOOP_SECRET` 兩個 env**。
...
| `MEMWAL_RECALL_CONCURRENCY` | 召回併發（SEAL 429 防護,預設 2） | |
...
| 變數 | 用途 | ★ |
|---|---|---|
| `WORLD_LOOP_URL` | = Vercel 的 `https://<web>/api/tick` | |
...
**現況**：`AdminPanel.tsx` 的開關是一個 local `useState(false)`,**純前端、沒接後端**。真正驅動自治的是 VPS 上的 world-loop 程式。

**要讓它變成真遙控（建議,很輕）**：
1. 在 relayer（或一個小端點）放一個持久旗標 `runnerPaused`：`GET /control` 回狀態、`POST /control` 寫值。
2. world-loop 每圈開頭讀 `GET /control`,`paused` 就跳過這次 tick。
3. admin 開關改成呼叫 `POST /control`,並輪詢 `GET /control` 顯示真實狀態（running / paused / last tick time）。
...
## 8. 首次部署 checklist
1. [ ] `publish` 合約到目標網路 → 更新 `contract-ids.ts` → commit。
2. [ ] Vercel：部署 `packages/web`,填上表 env（含 `TICK_LOOP_SECRET`、`MEMWAL_SERVER_URL` 暫留空/本機）。
3. [ ] 把 `packages/relayer` 加進 monorepo;Zeabur 建 service A（root=`packages/relayer`）+ 向量 store + Walrus URL → 拿到 relayer 網域。
4. [ ] 回填 web 的 `MEMWAL_SERVER_URL = https://<relayer>` 重新部署。
5. [ ] Zeabur 建 service B 跑 `world-loop`,填 `WORLD_LOOP_URL=https://<web>/api/tick` + `TICK_LOOP_SECRET`。
6. [ ] （選配）接 admin 開關 → relayer `/control` 旗標（§5）。
7. [ ] 煙霧測試：admin 手動「推進一個 tick」→ 看 POV/公報生成 → 確認 world-loop 自動跑起來。
