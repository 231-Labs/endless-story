...
6. **求助**，讓有需要的角色向同場且有能力的人開口。
7. **給予**，讓有能力的人決定是否伸出援手。
8. **結算**，在事件出牌之前處理薪餉、成本、已接受的接濟與程序內 economy adapter 的生存狀態。
9. **行動**，從事件允許的動作中自行選擇。
10. **出版**，產出角色 POV、已結算的事件章回，以及 Saga 的客觀公報。
11. **反思**，把近期零碎經驗整理成更耐久的記憶（睡眠整理在夜間執行）。

在移動與社交之間，預設路徑還會推導戲劇張力、開啟或延續 event spine，並在節奏需要時讓 Director 引入稀缺資源。目前預設 tick 已開啟感知、事件 spine 與 Director 建立新稀缺資源；並行事件、注意力耦合、LLM 框題與 rival-gravity 移動仍是可選控制項。
