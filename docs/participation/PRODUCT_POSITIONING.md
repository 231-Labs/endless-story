...
**短 tagline（對外、配品牌用）：「角色會記得，世界會生長。」** — 兩個動詞收住核心：角色有持久記憶（會記得）、世界自轉新陳代謝（會生長）；generic、無題材包袱，可與 `無盡敘界` 連讀。
（取代舊句「住在 Walrus 上的梨園」——舊句把產品鎖死在戲曲題材、又把基建名塞進標語，已棄用。）
...
> - **對外**叫 **春雪社**（看客模式：看戲／看世界／讀章回；次要入口：追蹤／訂閱／藏閣）。  
> - **對內**叫 **片場**（Cinema Lab：控拍、物界、訪談、匯出）。  
> - 同一套觀測台 UI，依身份顯示不同深度。Sui／Walrus 是後端實作，不是第二套網站（見 `packages/web/src/lib/ports`）。  
> - **劇院播放器** `HeroTheater` 保留，供影片／直播接回今日主鏡（不刪）。  
> - **第二期**：公開 API → Port；看客次要入口；舊 `/feed` `/dossier` `/saga` 降為鏈測深鏈。
