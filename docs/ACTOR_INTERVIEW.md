...
> **狀態**：**implemented（V1，含對照模式）** · 2026-07-22，多輪品質修正 #194（2026-07-23，見 §5）。本檔前半是當日的
...
**落地註（#194，訪談不再翻來覆去）**：小記憶池下同一角色連問幾輪會端出同一批 top-N，
答案於是繞著同幾句打轉。三處修正：

- **記憶輪換**：`buildInterviewContext(..., { excludeSeqs })` 收此場先前各輪已注入的
  `seq`，多召一輪候選後先端沒上過桌的；新的不夠才讓**至多 3 條**（`REPEAT_FLOOR`）舊的
  回鍋——她真只記得這些時就誠實見底，不硬湊。
- **後續輪瘦身**（`followUp`）：時刻是凍結的，開席那輪的世界素材（今日親歷／眼中眾人／
  心上懸著／眼下打算／模式脈絡）都還躺在 transcript 裡——原封重塞只會把她錨死在同幾句上。
  故第二輪起的 percept 只留一句「仍是○○，方才那席話還在繼續」＋**新翻出的**記憶＋問題，
  記憶標題也改成「心頭又浮起的記憶」；沒新記憶時明說「就憑方才所知回」。
- **反覆誦迴避**（`avoidEchoes`）：把她此場已出口的各輪答話餵給 beat 側同一支
  `recurringImagery`，抽出反覆的字句意象，以「【莫再重彈】」段指名列進 prompt
  （與場上文筆二階同一把尺，見 [`narrative/ENGINE_CORE.md`](./narrative/ENGINE_CORE.md) §3.1）。

煙囪驗證：同題連問四輪，召回逐輪輪替、答案各不相同。
