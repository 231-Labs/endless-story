 ...
 * character's snapshot. Idempotent per (sagaId, today): the day is only advanced once, so repeat
 * reads within a tick reuse the same result (and don't double-pay). `treasuryFundsEndless` is the
 * on-chain `Saga.treasury` in display ENDLESS units.
 */
export function settleSagaCohort(
    ...
    if (cached && cached.today === today) return cached.snaps;
