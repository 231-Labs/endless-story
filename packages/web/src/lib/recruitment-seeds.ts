/**
 * Canonical preset recruitments for the 春雪社 saga.
 *
 * Used by the admin "③ batch open" server action to populate the JSON
 * store. Living here (not under mocks/) because they're real production
 * seed data — admin clicks the button, these 5 jobs land in
 * `web/data/recruitments.json`, the carousel shows them.
 *
 * 5 archetypes covering the demo flow:
 *   武小生 / 富商 / 青衣 / 小報記者 / 老生
 */

import type { Recruitment } from '@endless-story/shared';

const DEMO_SAGA_ID = 'spring-snow';
const DEMO_SAGA_NAME = '春雪社';

/** TTL of seeded recruitments — pushes expiry 14 days out so demos don't
 *  immediately go stale. */
const SEED_TTL_DAYS = 14;

function expiresAt(): string {
    return new Date(Date.now() + SEED_TTL_DAYS * 86_400_000).toISOString();
}
...
export function buildDefaultRecruitments(): Recruitment[] {
    return [
        {
            id: 'rec_chunxue_wu_xiaosheng',
            sagaId: DEMO_SAGA_ID,
            sagaName: DEMO_SAGA_NAME,
            specialty: '武小生',
            roleIntent:
