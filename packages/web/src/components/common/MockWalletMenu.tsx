...
export function MockWalletMenu() {
  ...
  // Disconnected: the header pill IS the connect trigger — one click opens the
  // wallet picker directly, with no intermediate menu layer.
  if (!account) {
    return (
      <WalletConnectButton
        align="right"
        className="flex h-8 items-center gap-2 rounded-full bg-canvas/55 px-3 text-2xs tracking-widest text-ink/75 ring-1 ring-hairline transition-colors hover:text-ink hover:ring-ink/25 max-sm:w-8 max-sm:justify-center max-sm:px-0"
      >
        <span className="h-2 w-2 rounded-full bg-mute/50" />
        <span className="max-sm:hidden">未連接</span>
      </WalletConnectButton>
    );
  }

  // Connected: the account dropdown (balance, drip, viewer links, disconnect).
  const dossierHref = `/dossier?filter=mine&as=${account.address}`;
  const chamberHref = `/chamber?id=${account.address}`;
  ...
  return (
    <details className="group/menu relative">
      <summary className="flex h-8 cursor-pointer list-none items-center gap-2 rounded-full bg-canvas/55 px-3 text-2xs tracking-widest text-ink/75 ring-1 ring-hairline transition-colors hover:text-ink hover:ring-ink/25 max-sm:w-8 max-sm:justify-center max-sm:px-0 [&::-webkit-details-marker]:hidden">
        <span className="h-2 w-2 rounded-full bg-jade shadow-[0_0_12px_rgba(142,172,138,0.35)]" />
        <span className="max-sm:hidden">{truncateAddress(account.address, 4, 4)}</span>
      </summary>
