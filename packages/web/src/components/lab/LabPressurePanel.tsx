...
export function LabPressurePanel({
    ...
    return (
        <div
            className={`pointer-events-auto absolute right-3 z-30 w-[min(22rem,calc(100%-1.5rem))] sm:right-5 sm:top-5 ${
                shiftDown ? 'top-28' : 'top-3'
            }`}
        >
            <div className="overflow-hidden rounded-xl border border-hairline bg-canvas/80 shadow-[0_4px_20px_rgba(20,12,8,0.14)] backdrop-blur-md dark:bg-black/60">
                <button
                    ...
                    title={`外力牌組「${pressure.deckId}」：結帳死線、發俸分紅、導演選牌，以及角色自己做得出來的世情動作`}
                >
                ...
                {/* 誰打的 —— always visible: the authority split is the whole design */}
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 px-3 pb-2.5 pt-1 font-serif text-2xs tracking-[0.12em] text-mute">
                ...
                {open ? (
                    <div className="max-h-[26rem] overflow-y-auto border-t border-hairline px-3 py-2.5">
                        ...
                        {vitals ? (
                            <dl className="space-y-1 font-serif text-2xs tracking-[0.1em] text-mute">
                                <Row
                                    label="不可逆"
