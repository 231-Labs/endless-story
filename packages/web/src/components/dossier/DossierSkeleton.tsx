...
/**
 * Full-screen dossier skeleton. Shared by:
 *   - the route's `loading.tsx` (Suspense fallback on cold navigation)
 *   - the per-character `<Suspense key={id}>` in the page (so switching
 *     character → character also shows it, not just first entry)
 *
 * Mirrors the DossierHeader layout (portrait + name/meta + chips +
 * live-state bar) so the swap to real content doesn't jump.
 */
      ...
      <div className="flex min-h-[calc(100dvh-var(--es-site-nav-h))] flex-col justify-center pb-16">
        <div className="w-full px-5 sm:px-10">
          <div className="mx-auto max-w-6xl animate-pulse">
            <Bar className="mb-10 h-4 w-32" /> {/* back link */}
