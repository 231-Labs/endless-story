/**
 * Self-contained AI client for the season harness — mirrors @endless-story/llm's
 * pattern (same env vars, same provider precedence, same model defaults) but with
 * ZERO cross-package imports, so the harness runs under plain `node` (native TS
 * type-stripping) with no install and no tsx, for BOTH mock and real keys.
 *
 * (App code — web/runner/cli — must still go through @endless-story/llm. This is a
 *  standalone verification harness, like packages/economy and packages/troupe, so a
 *  small built-in client is the right call.)
 *
 * Key via env (drop packages/season/.env.local, reuse ../web/.env.local, or inline):
 *   ZAI_API_KEY | POE_API_KEY | ANTHROPIC_API_KEY   (+ AI_PROVIDER to force one)
 * No key (or --mock / SEASON_MOCK=1) → deterministic local model, whole loop runs.
 */
...
/**
 * One logical call, made robust: try the primary model, then on a transient error
 * (timeout/429/5xx) back off and fall to the faster cheap model (twice). A hard error
 * (401/400) throws immediately — no point retrying a bad key.
 */
...
export function makeAsk(opts: { mock?: boolean } = {}): Ask {
  const forceMock = opts.mock || process.env.SEASON_MOCK === '1';
  const cfg = forceMock ? null : resolveProvider();

  if (!cfg) {
    return attachAsk(async (_req) => {
      throw new Error('ask() called in mock mode — use askOr()/askJson() so the deterministic model runs');
...
/** Run the LLM if wired, else fall back to the deterministic model. */
...
/**
 * Run `fn` over items with a concurrency cap (preserves order). Keeps a season's
 * fan-out (N characters × M scenes) from firing many simultaneous requests (429s).
 */
