/**
 * Spatial routing — night-time placement substrate (§2.50). Pure math, no I/O.
 * Tired characters go to their own home; following someone into a private home
 * needs the owner's welcome, so privacy EMERGES. By day the router is silent and
 * the LLM keeps agency. The character→home map (durable store) lives with the
 * caller (web keeps a file-store; the engine keeps it in WorldState); only the
 * placement math lives here.
 */
