 ...
 * 
 * The narrative engine already paints event-moment images
 * (角色 gallery 的 event_moment, stored on Walrus). This module mints such a
 * moment as an ownable, Kiosk-tradeable NFT — the demand side of the character
 * economy: a fan buys the 劇照 of a scene they love; the proceeds can flow back to
 * the saga treasury / the character (royalty wiring is a follow-up — see
 * docs/CHARACTER_ECONOMY.md).
 * 
 * Authority: minting is StorytellerCap-gated (the troupe issues its own戲單/劇照),
 * mirroring scene/event creation. Trading is plain Kiosk — `Still` has
 * `key + store`, no transfer policy gymnastics in v1.
 * 
...
export interface MintStillArguments {
    ...
    edition: RawTransactionArgument<number | bigint>;
}
...
/**
 * Mint a 劇照 from an event moment. Returns the Still so the PTB can transfer it
 * to a recipient or place it straight into a Kiosk.
 */
export function mintStill(options: MintStillOptions) {
    ...
    const parameterNames = ["cap", "saga", "characterId", "walrusBlobId", "imageUrl", "title", "edition"];
