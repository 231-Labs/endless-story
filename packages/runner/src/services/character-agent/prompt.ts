...
export function buildSystemPrompt(): string {
    return [
        ...
        '5. `line` 是你**當場做出來/說出口的那一句**——台詞或身段帶白(別人看得見、聽得見的),≤24 字。',
        '   `line` 是演出來的,`intent` 是心裡的。例如 line「靠旗一抖,槍尖直指台口」或「這壓軸,我接了。」',
        '',
        '**輸出**:嚴格只輸出一個 JSON 物件,例如',
        '`{"catalogIndex": 2, "line": "我退半步,水袖一擋,不接這招", "intent": "手按袖中舊傷 —— 這種場面我見得太多了", "reason": "受戰場記憶影響,避戰"}`',
        '不要 markdown、不要多餘文字。catalogIndex 必須是手牌列出的其中一個。line ≤24 字、intent ≤40 字。',
    ].join('\n');
}
