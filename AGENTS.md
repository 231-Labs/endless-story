...
- **ASK 進迴圈 + 死亡有後果（2026-06-11）— e2e 可看出效果**：tick-loop 加 **2.85 ASK phase**（`tick-phases/ask.ts`，排在 SOCIAL 後 GIVE 前）：needy 同場角色用 `decideAskAction` 向同場 solvent 候選開口求助 → 寫 memory/scene-line → `asksByGiver` 傳給 GIVE，被求的 giver 在 peer 名單看到 `loan-request`＋askAmount（拉路接通推路）。give/ask 共用判斷抽到 `tick-phases/econ-helpers.ts`。**死亡有後果**：`saga-economy.ts` `isShadowDead`，orchestrator 把影子已死的人剔出 active slice（不再 plan/move/act），dossier 仍顯示 `vitalityState='dead'`。`TickLoopResult.asks` + SchedulerPanel「求助」區 + SETTLE「殞」名單。整條經濟迴圈：**發薪→扣記憶租金→開口求助→接濟轉帳(仇家可拒)→老死/餓死退場**，全 off-chain、world-loop→`/api/tick` 同進程可 e2e。demo 提示：要看到接濟/求助，需角色 survival 有貧富差（給部分角色訂閱→perf bonus 變富、或記憶/年齡拉開 cost）；金庫太大→人人 healthy 無人求，金庫空→salary 0 人人窮。web type-check 我方檔零錯、economy 41 綠、runner 40 綠。
...
- **角色所有權 + 非 admin mint 已改成鏈上正統兩段式（2026-06-11，待 redeploy）**：`mint_character_internal` 仍在合約內把 `OwnerCap` `public_transfer` 給 `owner_recipient`（redeem 路徑＝原 voucher.payer），只回 `(ID, ControlCap)`；另新增 `RedeemIntent`：user 接受 preview 後先用自己錢包 `request_redeem_voucher(voucher)`，把 user-owned voucher 消耗成 shared intent；server/admin 再用 `redeem_intent_to_character(intent)` + `StorytellerCap` mint。這修掉 admin server action 直接消耗 user-owned voucher 造成的 `Transaction was not signed by the correct sender`。`sui move test` 98/98 綠；`@endless-story/sdk type-check` 綠；`web type-check` 仍只卡既有 `@endless-story/chamber-3d` resolution。codegen 已跑過並更新 `packages/sdk/src/generated/endless_story/recruit.ts` 與 `contracts/endless_story/package_summaries`。
