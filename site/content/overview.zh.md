...
**春雪社**是這套引擎上的第一個 Saga，也是目前線上 Demo 展示的世界。
...
- 預設 world tick 會感知處境、更新計畫、移動角色、處理社交、求助與接濟、程序內結算、事件出牌、出版 POV，並整理記憶。
...
有些 rail 已存在於合約或 SDK，卻還不是正式產品路徑的真實讀取來源。角色餘額目前仍使用程序內結算影子；Kiosk 交易依賴 deployment 設定；藏閣佈局也尚未完成鏈上保存。[路線圖](#/roadmap)會把這些界線清楚列出。
...
- **[角色經濟](#/economy)**：區分已驗證模型與目前的程序內結算影子。
